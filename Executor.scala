...
import java.io.{EOFException, FileInputStream, FileOutputStream, NotSerializableException, ObjectInputStream}
import java.io.File
 ...
 */
// do i send shit as env and then i cannot create a serializer ?
private[spark] class Executor(
    ...
    isSeanet: Boolean = false)
  extends Logging {

  logInfo(s"Starting executor ID $executorId on host $executorHostname am i seanet ? " + isSeanet)
  ...
  private var heartbeatReceiverRef: RpcEndpointRef = _
  ...
  def launchTask(context: ExecutorBackend, taskDescription: TaskDescription,isSeanet: Boolean = false, fileIndex: Int = 0): Unit = {
    val tr = new TaskRunner(context, taskDescription, plugins, isSeanet, fileIndex)
    runningTasks.put(taskDescription.taskId, tr)
    System.out.println("Seanet inside launch task of  executor")
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      if (isSeanet) {
        ...
        logInfo("seanet directory exists in executor run")
        ...
        logInfo(s"seanet starting  $taskName which is a " )

        // seanet here 1/3/2023
        try {
          // original
/*
        synchronized {
...
*/
          ...
          // those files contain serializedTasks
          // seanet here 26/3
          val myDir = new File("../../../taskDescTaskFiles")
          val taskBinaries: List[File] = myDir.listFiles().filter(name => name.getName.startsWith("taskDescTaskBuffer")).sorted.toList
          val file = taskBinaries.apply(changingIndex)
          val obs = new ObjectInputStream(new FileInputStream(file))
          val taskDescBuffer = readObject(obs)
          //  it is a serializedTask becoming a result or other task
          task = ser.deserialize[Task[Any]](taskDescBuffer.asInstanceOf[ByteBuffer], Thread.currentThread.getContextClassLoader)
        } catch {
          case e: Exception => {
          ...
          logInfo(s"seanet task created " + task)
          task.localProperties = taskDescription.properties
          logInfo(s"seanet properties fine")
          val res = task.run(
            ...
            plugins = plugins,
            true)
          logInfo(s"seanet task run")
          System.out.println("seanet task run")
          val valueBytes = resultSer.serialize(res)
          var lala: Array[Long] = Array.empty
          val directResult = new DirectTaskResult(valueBytes, accumUpdates, lala)
          logInfo(s"seanet direct result made")
          System.out.println("seanet direct result made")

          val serializedDirectResult = ser.serialize(directResult)
          val out = new FileOutputStream("taskBinary"+ taskDescription.taskId +".rsl");
          Utils.writeByteBuffer(serializedDirectResult, out)
          val result = ser.deserialize[DirectTaskResult[Int]](serializedDirectResult)
          System.out.println("result of " + taskDescription.taskId + " is " + result.value())

        logInfo("seanet has completed running all of the files");
        // } // synchronized
      ...
      try {
        ...
        // seanet here just use this line for a dummy run
        execBackend.statusUpdate(taskId, TaskState.FINISHED, serializedResult)
      } catch {
    ...
    private def readObject(in: ObjectInputStream): ByteBuffer = Utils.tryOrIOException {
      ...
      logInfo("seanet read int :" + length)
