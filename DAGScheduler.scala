...
import java.io.{File, FileOutputStream, NotSerializableException, ObjectOutputStream}
...
private[spark] class DAGScheduler(
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int): Unit = {
    ...
    try {
      ...
      RDDCheckpointData.synchronized {
        taskBinaryBytes = stage match {
          case stage: ShuffleMapStage =>
            ...
            val both = JavaUtils.bufferToArray(
              closureSerializer.serialize((stage.rdd, stage.shuffleDep): AnyRef))

            val fileDesc = new File("broadcasts/filedesc_" + fileIndex +  ".bdc")
            val os1ab = new ObjectOutputStream(new FileOutputStream(fileDesc))
            os1ab.writeChars(stage.rdd.getClass.getName + "\n")
            os1ab.writeChars(stage.shuffleDep.getClass.getName)
            os1ab.close()

            val fileActual = new File("broadcasts/rddonly_" + fileIndex +  ".bdc")
            val os1a = new ObjectOutputStream(new FileOutputStream(fileActual))
            os1a.writeObject(stage.rdd)
            os1a.close()

            if (stage.rdd.isInstanceOf[MapPartitionsRDD[_,_]]) {
              val fileActual = new File("broadcasts/rddfucntiononly_" + fileIndex +  ".bdc")
              val field: Field = classOf[ResultStage].getDeclaredField("f")
              field.setAccessible(true)
              val fValue = field.get(stage.rdd).asInstanceOf[(TaskContext, Int, Iterator[Any]) => Iterator[Any]]
              val os1a = new ObjectOutputStream(new FileOutputStream(fileActual))

              os1a.writeObject(fValue)
              os1a.close()
            }


            val fileActual2 = new File("broadcasts/funconly_" + fileIndex +  ".bdc")
            val os1a2 = new ObjectOutputStream(new FileOutputStream(fileActual))
            os1a2.writeObject(stage.shuffleDep)
            os1a2.close()
            ...
            case stage: ResultStage =>
              ...
              val file = new File("broadcasts")
              file.mkdirs

             /* val fileActual = new File("broadcasts/stagesntRDD_" + fileIndex +  ".bdc")
              val os1a = new ObjectOutputStream(new FileOutputStream(fileActual))

              os1a.writeObject(stage.rdd)
              os1a.close()

              val fileActual2 = new File("broadcasts/stagesntfunc_" + fileIndex +  ".bdc")
              val os1b = new ObjectOutputStream(new FileOutputStream(fileActual2))
              os1b.writeObject(stage.func)
              os1b.close()*/


              var both: Array[Byte] =null.asInstanceOf[Array[Byte]]

              val fileDesc = new File("broadcasts/filedesc_" + fileIndex +  ".bdc")
              val os1ab = new ObjectOutputStream(new FileOutputStream(fileDesc))
              os1ab.writeChars(stage.rdd.getClass.getName + "\n")
              os1ab.writeChars(stage.func.getClass.getName)
              os1ab.close()
              ...
              if (stage.rdd.isInstanceOf[MapPartitionsRDD[_,_]]) {
                ...
                val fileActual2 = new File("broadcasts/funconly_" + fileIndex +  ".bdc")
                val os1a2 = new ObjectOutputStream(new FileOutputStream(fileActual2))

                os1a2.writeObject(stage.func)
                os1a2.close()


                System.out.println("seanet dagscheduler stage rdd is a mpapartitionrdd")
                System.out.println("seanet dagscheduler stage rdd is a " + stage.rdd.getClass)
                both = JavaUtils.bufferToArray(closureSerializer.serialize((stage.rdd, stage.func): AnyRef))
                both
              }
      ...
      logInfo("Seanet Dagscheduler write blockManager " + (SparkEnv.get.blockManager != null && SparkEnv.get.blockManager.diskBlockManager != null))

      // i write the same thiong twice in a different format
      val fileActual2 = new File("broadcasts/stagesntTaskBinaryToBroadcast_" + fileIndex +  ".bdc")
      val os1a2 = new ObjectOutputStream(new FileOutputStream(fileActual2))
      os1a2.writeObject(taskBinaryBytes)
      os1a2.close()


      val fileActual = new File("broadcasts/stagesntRDDActualBoth_" + fileIndex +  ".bdc")
      val os1a = new ObjectOutputStream(new FileOutputStream(fileActual))
      os1a.writeObject(taskBinary)
      os1a.close()
      fileIndex += 1
      ...
      stage match {
        case stage: ShuffleMapStage =>
          ...
          partitionsToCompute.map { id =>
            ...
            val this_task: ShuffleMapTask = new ShuffleMapTask(stage.id, stage.latestInfo.attemptNumber,
