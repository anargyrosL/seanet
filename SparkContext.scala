...
class SparkContext(config: SparkConf) extends Logging {
  ...
  def broadcast[T: ClassTag](value: T): Broadcast[T] = {
    ...
    // seanet done, write broacast to file including STAGES
/*    logInfo("seanet newage writing broadcast thing to a file")
    val file = new File("broadcasts")
    file.mkdirs
    val fileActual = new File("broadcasts/stagesnt_" + fileIndex + "_" + value.toString + ".bdc")
    val os1a = new ObjectOutputStream(new FileOutputStream(fileActual))
    fileIndex = fileIndex + 1
    os1a.writeObject(bc)
    os1a.close()*/
    // logInfo("seanet new age just wrote the broadcast files")
...
object SparkContext extends Logging {
  ...
  def getOrCreate(config: SparkConf): SparkContext = {
    try {
      if (config.get("amiseanet").equals("yes")) {
        if ( activeContext.get() != null) {
          return activeContext.get()
        }
      }
    } catch {
      case e: Throwable => logWarning("amiseanet is not there you can safely ignore this message")
    }
