...
import org.apache.spark.scheduler.{DirectTaskResult, ExecutorLossMessage, ExecutorLossReason, TaskDescription}
import org.apache.spark.scheduler.cluster.CoarseGrainedClusterMessages._
import org.apache.spark.util.{AccumulatorContext, AccumulatorMetadata, ChildFirstURLClassLoader, LongAccumulator, MutableURLClassLoader, SignalUtils, ThreadUtils, Utils}
...
/*    import java.io.FileOutputStream
    ...
    val os1 = new ObjectOutputStream(new FileOutputStream("taskBinary" + fileIndex + ".bin"))
    fileIndex = fileIndex + 1
    val properties = new Properties()
    properties.put("lala", "null")
    val lala = new TaskDescription(666, 666, "666", "666", 666, 666,
      null, null, null, properties, 7, null, null);
    val encoded: ByteBuffer = encode(lala)
    os1.writeInt(encoded.array().length)
    os1.write(encoded.array())
    os1.close()*/
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case LaunchTask(data) =>
      // we should have all of the necessary data here, seanet

        //if dir count words exists start making the program
        if (new File("../../../count_words").exists()) {
          logInfo("seanet worker found the count words dir")
          val myDir = new File("../../../count_words")
          val taskBinaries:List[File] = myDir.listFiles().filter(name => name.getName.startsWith("taskBinary")).sorted.toList
          logInfo("seanet taskBinaries are: " + taskBinaries)
          taskBinaries.map(file => new ObjectInputStream((new FileInputStream(file)))).foreach(inputStream =>
            try{
              logInfo("seanet launching ready made tasks for " )
              val size = inputStream.readInt()
              val array: Array[Byte] = new Array[Byte](size)
              inputStream.read(array)
              inputStream.close()
              val taskDesc = TaskDescription.decode(ByteBuffer.wrap(array))
              logInfo("seanet decoded data from file")
              executor.heartbeater.start()
              executor.launchTask(this, taskDesc, false)
              //statusUpdate( taskDesc.taskId, TaskState.FINISHED, serializedDirectResult)
            } catch {

              case e: IOException => {
                logInfo("seanet exception in worker trying to read autonomously")
      ...
      val os1 = new ObjectOutputStream(new FileOutputStream("taskBinary" + fileIndex + ".bin"))
      fileIndex = fileIndex + 1
      os1.writeInt(data.buffer.array().length)
      os1.write(data.buffer.array())
      os1.close()
        ...
        if (executor == null) {
          ...
          logInfo("seanet New version 6. Got assigned task " + taskDesc.taskId)


          taskResources(taskDesc.taskId) = taskDesc.resources
          // executor.launchTask(this, taskDesc)
          // HERE
          log.info("seanet New version 6 backend executor launch task doing nothing BUT")
          logInfo("seanet New version 6 backend executor writing to file the task i got assigned")
          logInfo("seanet New version 6 backend executor taskDescription: " + taskDesc.toString)

          val thisIndex = fileIndex -1
          val os2 = new ObjectInputStream(new FileInputStream("taskBinary" + thisIndex  + ".bin"))
          val size = os2.readInt()
          val array: Array[Byte] = new Array[Byte](size)
          os2.read(array)
          os2.close()

          val buffer = ByteBuffer.wrap(array)
          val decoded = TaskDescription.decode(buffer)
          logInfo("seanet read task " + decoded)

          // Utils.writeByteBuffer(TaskDescription.encode(taskDesc), out)
          ...
          val accumUpdates = Seq()
          // val metricPeaks = metricsPoller.getTaskMetricPeaks(taskId) */
          val emptyArray = Array[Long]()
          // TODO: do not serialize value twice
          val directResult = new DirectTaskResult(valueBytes, accumUpdates, emptyArray)
          val ser = env.closureSerializer.newInstance()
          val serializedDirectResult = ser.serialize(directResult)
          val resultSize = serializedDirectResult.limit()
          Thread.sleep(500)
          executor.setTr(this, taskDesc)
          executor.tr.setTaskFinishedAndClearInterruptStatus()
          statusUpdate( taskDesc.taskId, TaskState.FINISHED, serializedDirectResult)
        }
        }
  ...
  override def statusUpdate(taskId: Long, state: TaskState, data: ByteBuffer): Unit = {
    logInfo("seanet statusUpdate from coarsegrainedexecutorbackend")
    ...
    driver match {
      case Some(driverRef) => {
        logInfo("seanet statusUpdate sending message with state isFinished "+ TaskState.isFinished(state) + " with state "+ state + " to driver")
        logInfo("seanet DRIVERREF " + driverRef.address + " " + driverRef)
        driverRef.send(msg)
      }
