...
import org.apache.spark.deploy.{ApplicationDescription, Command, ExecutorDescription, ExecutorState}
...
private[deploy] class Worker(
  ...
  override def onStart(): Unit = {
    ...
    metricsSystem.start()
    if (isSeanet) {
      // do stuff about seanet
      logInfo("Worker starting in seanet mode")
      //val cmd = new Command("mainClass", List("arg1", "arg2"), Map(), Seq(), Seq(), Seq())
      //seanet edw
      val args = Seq(
        "--driver-url", "spark://seanet:666",
      ...
      val executorEnvs = HashMap[String, String]()
      executorEnvs("SPARK_USER") = "argyris"
      //seanet edw: an valw se optargs to seanet mporw na kanw lauch task meta
      val cmd = Command("org.apache.spark.executor.CoarseGrainedExecutorBackend",
        args, executorEnvs, Seq.empty, Seq.empty, Seq.empty)
      val appDesc = new ApplicationDescription("name", Some(4), 1234, cmd, "appUiUrl")
      receive(LaunchExecutor("seanet/skata", "667", 667, appDesc, 667, 667, Map.empty))
      // seanet useful this could be of use: private[spark] trait RpcEnvFileServer {
    } else {
      logInfo("Worker starting the webUI")
    ...
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_, resources_) =>
      /*if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          var manager: ExecutorRunner = null;
          if (isSeanet) {
            logInfo("seanet rigged executor runner starting")
            manager = new ExecutorRunner(
              appId,
              execId,
...
private[deploy] object Worker extends Logging {
  ...
  def startRpcEnvAndEndpoint(
    ...
    logInfo("senaet in worker i find host to be " + host + " master " + masterAddresses(0).asInstanceOf[RpcAddress].toString.contains("seanet"))
    // senaet here
    if (!masterAddresses(0).asInstanceOf[RpcAddress].toString.contains("seanet")) {
      logInfo("senaet in worke calling new with isseanet false")
      rpcEnv.setupEndpoint(ENDPOINT_NAME, new Worker(rpcEnv, webUiPort, cores, memory,
        masterAddresses, ENDPOINT_NAME, workDir, conf, securityMgr, resourceFileOpt))
    } else {
      conf.setJars(Seq("/mnt/c/Users/argyris/Desktop/homebrew/spark-3.2.1/spark-3.2.1/assembly/target/scala-2.12/jars/*",
        "/mnt/c/Users/argyris/Desktop/homebrew/spark-3.2.1/spark-3.2.1/core/target/jars/*",
        "/mnt/c/Users/argyris/Desktop/homebrew/spark-3.2.1/spark-3.2.1/core/target/*"))
      logInfo("senaet in worke calling new with isseanet true")
      rpcEnv.setupEndpoint(ENDPOINT_NAME, new Worker(rpcEnv, webUiPort, cores, memory,
        masterAddresses, ENDPOINT_NAME, workDir, conf, securityMgr, resourceFileOpt, null,true))
    }
    rpcEnv
  }
